                            ...
                            <div class="uk-margin" v-if="session_profile == 'Admin'">
                                ...
                                <div class="uk-form-controls">
                                    <select class="uk-select" v-model="type">
                                        <option v-for="t in type_list" :value="t.id" :key="t.id">
                                        {{t.id}}
                                        </option>
                                    </select>
                                </div>
...
export default {
    ...
    return {
      columnDefs: [
        ...
        },
        {
          headerName: 'Mobile',
          minWidth: 200,
          valueGetter: this.mobileValueGetter,
          headerClass: 'resizable-header',
        },
        {
          headerName: 'Type',
      ...
      rowData2: [],
      gridOptions2: {
        onRowClicked: this.RowClickEventHandler2,
        rowSelection: 'single',
        suppressPropertyNamesCheck: true,
        enableColResize: true,
        onGridReady: function(params) {
  ...
  methods: {
    ...
    RowClickEventHandler(event) {
      // console.log(event);
      this.rowData[event.rowIndex].selected = !this.rowData[event.rowIndex]
        .selected;
      console.log(this.rowData);
      this.gridOptions.api.setRowData(this.rowData);
    },
    RowClickEventHandler2(event) {
      // console.log(event);
      this.rowData2[event.rowIndex].selected = !this.rowData2[event.rowIndex]
        .selected;
    ...
    getProfile() {
      ...
      console.log(request);
      axios
        .post(this.$store.state.api_url + '/user_profile', request)
        .then(response => {
    ...
    getReps() {
      ...
      console.log(request);
      axios
        .post(this.$store.state.api_url + '/user_profile_rep_list', request)
        .then(response => {
          console.log(response);
    ...
    getFarms() {
      ...
      axios
        .post(
          this.$store.state.api_url + '/user_profile_wine_farm_list',
          request
        )
        .then(response => {
          console.log(response);
    ...
    createProfile() {
      ...
      if (this.type == 'Farm') {
        request.farms = this.rowData2;
      }
      console.log('Request: ', request);
      axios
        .post(this.$store.state.api_url + '/user_profile_update', request)
        .then(response => {
