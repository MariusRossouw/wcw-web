                                ...
                                <div class="uk-grid-small" style="padding-right: 15px;" uk-grid>
                                    ...
                                    <div class="uk-width-1-6@m">
                                        <div class="uk-card uk-card-primary uk-card-small">
                                            ...
                                            <div class="cardBody">
                                                <input v-model="search" placeholder="Search" style="margin-left: 28px; width: 230px; height: 35px;"/>
                                                <button v-for="(m, index) in filteredList" :key="index" :class="{'isSelected': m['selected'],  'isNotSelected': !m['selected']}" @click="updateMerchants(index, m)">{{m.merchant_name}}</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="uk-width-1-6@m">
      ...
      columnDefs: [
        {
          ...
          filterParams: {
            ...
            textCustomComparator: function(filter, value, filterText) {
              ...
              return (
                literalMatch ||
                contains(valueLowerCase, aliases[filterTextLoweCase])
              );
            },
            debounceMs: 2000,
          },
  ...
  methods: {
        ...
        if (this.merchants[k].merchant_id == merchant.merchant_id) {
          ...
          if (this.merchants[k].selected) {
            ...
            for (var i = 0; i < this.filters.merchant_filter.length; i++) {
              if (
                this.filters.merchant_filter[i].merchant_id ===
                this.merchants[k].merchant_id
              ) {
        ...
        if (this.products[k].product_id == product.product_id) {
          ...
          if (this.products[k].selected) {
            ...
            for (var i = 0; i < this.filters.products.length; i++) {
              if (
                this.filters.products[i].product_id ===
                this.products[k].product_id
              ) {
    ...
    updateYears(index, year) {
      ...
      if (this.years[index].selected) {
        this.filters.years.indexOf(year) === -1
    ...
    updateMonths(index, month) {
      ...
      if (this.months[index].selected) {
        this.filters.months.indexOf(month) === -1
    ...
    updateMerchantGroups(index, merchant_group) {
      this.merchant_groups[index].selected = !this.merchant_groups[index]
        .selected;
    ...
    getFilterData() {
      ...
      axios
        .post(this.$store.state.api_url + 'filters', request)
        .then(response => {
          ...
          for (var i = 0; i < response.data.data.products.length; i++) {
            response.data.data.products[
              i
            ].product_name = response.data.data.products[
              i
    ...
    getBudgetAndSales() {
      ...
      axios
        .post(this.$store.state.api_url + 'transactions_budget', request) //transactions_filtered
        .then(response => {
          ...
          for (var i = 0; i < response.data.data.years.length; i++) {
            for (
              var j = 0;
              j < response.data.data.years[i].volume.length;
              j++
            ) {
              if (
                response.data.data.years[i].volume[j] > this.topVolumeAmount
              ) {
                this.topVolumeAmount =
                  Math.ceil(response.data.data.years[i].sale[j] / 10000) * 400;
                this.topVolumeSpace = this.topVolumeAmount / 5;
              }
            }
            for (
              var j = 0;
              j < response.data.data.years[i].volume_accum.length;
              j++
            ) {
              if (
                response.data.data.years[i].volume_accum[j] >
                this.topVolumeAccume
              ) {
                this.topVolumeAccume =
                  Math.ceil(
                    response.data.data.years[i].volume_accum[j] / 10000
                  ) * 10000;
            ...
            for (var j = 0; j < response.data.data.years[i].sale.length; j++) {
              if (response.data.data.years[i].sale[j] > this.topSaleAmount) {
                this.topSaleAmount =
                  Math.ceil(response.data.data.years[i].sale[j] / 10000) *
                  10000;
            ...
            for (
              var j = 0;
              j < response.data.data.years[i].budget.length;
              j++
            ) {
              if (response.data.data.years[i].budget[j] > this.topSaleAmount) {
                this.topSaleAmount =
                  Math.ceil(response.data.data.years[i].budget[j] / 10000) *
                  10000;
                this.topSaleSpace = this.topSaleAmount / 5;
              }
            }
            for (
              var j = 0;
              j < response.data.data.years[i].sale_accum.length;
              j++
            ) {
              if (
                response.data.data.years[i].sale_accum[j] >
                this.topSaleAmountAccume
              ) {
                this.topSaleAmountAccume =
                  Math.ceil(response.data.data.years[i].sale_accum[j] / 10000) *
                  10000;
            ...
            for (
              var j = 0;
              j < response.data.data.years[i].budget_accum.length;
              j++
            ) {
              if (
                response.data.data.years[i].budget_accum[j] >
                this.topSaleAmountAccume
              ) {
                this.topSaleAmountAccume =
                  Math.ceil(
                    response.data.data.years[i].budget_accum[j] / 10000
                  ) * 10000;
                this.topSaleSpaceAccume = this.topSaleAmountAccume / 5;
              }
            }
    ...
    getTypes() {
      ...
      // console.log("Request: " + JSON.stringify(request));
      axios
        .post(this.$store.state.api_url + 'transactions_types', request) //transactions_filtered
        .then(response => {
    ...
    getProvinces() {
      ...
      // console.log("Request: " + JSON.stringify(request));
      axios
        .post(this.$store.state.api_url + 'transactions_provinces', request) //transactions_filtered
        .then(response => {
          // console.log('Response: ', response);
          this.loading3 = false;
    ...
    getCodes() {
      ...
      // console.log("Request: " + JSON.stringify(request));
      axios
        .post(this.$store.state.api_url + 'transactions_codes', request) //transactions_filtered
        .then(response => {
          // console.log('Response: ', response);
          this.loading4 = false;
    ...
    getTop5Products() {
      ...
      // console.log("Request: " + JSON.stringify(request));
      axios
        .post(this.$store.state.api_url + 'transactions_top5_products', request) //transactions_filtered
        .then(response => {
          // console.log('Response: ', response);
    ...
    getBottom5Merchants() {
      ...
      axios
        .post(
          this.$store.state.api_url + 'transactions_bottom5_merchants',
          request
    ...
    getTop5Merchants() {
      ...
      axios
        .post(
          this.$store.state.api_url + 'transactions_top5_merchants',
          request
        ) //transactions_filtered
        .then(response => {
          // console.log('Response: ', response);
    ...
    getBottom5Products() {
      ...
      axios
        .post(
          this.$store.state.api_url + 'transactions_bottom5_products',
          request
        ) //transactions_filtered
        .then(response => {
          // console.log('Response: ', response);
    ...
    getGraphData() {
      ...
      // console.log("Request: " + JSON.stringify(request));
      axios
        .post(this.$store.state.api_url + 'transactions_grid', request) //transactions_filtered
        .then(response => {
          // console.log('Response: ', response);
          this.gridOptions.api.setColumnDefs(this.columnDefs);
